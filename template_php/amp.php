    ...
    <body>
        <article id="preview">
            ...
            <div class="content">
                ...
                <?php
                $Read->ExeRead("ws_posts", "WHERE post_status = 1 AND post_id != :id AND post_category = :cat ORDER BY rand() LIMIT 10", "id={$post_id}&cat={$post_category}");
                if ($Read->getResult()):

                    $Arr = null;
                    foreach ($Read->getResult() as $more):
                        $Arr['items'][] = [
                            'url' => BASE . '/artigo/' . $more['post_name'] . '/amp',
                            'thumbnail' => BASE . '/uploads/' . $more['post_cover'],
                    ...
                    $Arquivo = $Pasta . Check::Name($SEO->getTitle()) . '.json';
                    $fp = fopen($Arquivo, 'w+');
                    fwrite($fp, json_encode($Arr));
                    fclose($fp);
                    ?>
                    <amp-list width="300" height="75" layout="responsive" src="<?= BASE . '/' . $Arquivo; ?>">
                        <template type="amp-mustache">
                            <a class="card related" href="{{url}}">
                                <amp-img width="130" height="75" src="{{thumbnail}}"></amp-img>
                                <span>{{title}}</span>
                            </a>
                        </template>
                    </amp-list>
